	...
	class CFdefault{
		public:
			...
			static int gcons_default(const std::vector<T> &arg_vec, ConfigUnit *conf)
			{
				...
				if(arg_vec.size() != 1){
					ul_writelog(Log::warning(), 
							"Constraint: wrong arg number %u for global constraint gcons_default(), 1 arg is required.", 
							(unsigned int)arg_vec.size());
					return -1;
				}
				str_t def_value;
			...
			static int gcons_enum(const std::vector<T> &arg_vec, ConfigUnit *conf)
			{
				...
				if(arg_vec.size() == 0){
					ul_writelog(Log::warning(), 
							"Constraint: wrong arg number 0 for global constraint gcons_enum(), at least 1 arg is required."); 
					return -1;
				}
			...
			static int gcons_range(const std::vector<T> &arg_vec, ConfigUnit *conf)
			{
				if(typeid(T) == typeid(str_t)){
					ul_writelog(Log::warning(), 
							"Constraint: gcons_range() does not support str_t type constraint");
					return -1;
				}
				if(arg_vec.size() != 2){
					ul_writelog(Log::warning(), 
							"Constraint: wrong arg number %u for global constraint gcons_range(), 2 args are required.", 
							(unsigned int)arg_vec.size());
					return -1;
				}
			...
			static int cb_check_range(ConfigUnit *unit, void *range_vec)
			{
				...
				int errcode = 0;
				ul_writelog(Log::warning(), 
						"Constraint: gcons_range() [key : value]:[%s : %s]: value not in range.", 
						unit->get_key_name().c_str(), unit->to_cstr(&errcode, "ERROR"));
				return -1;
			}
	};
}
